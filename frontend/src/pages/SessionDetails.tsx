...
export default function SessionDetails() {
  ...
  const [comparingCandidates, setComparingCandidates] = useState<
    ApiCandidate[]
  >([]);
  ...
  const handleToggleSelect = (candidate: ApiCandidate) => {
  ...
  const handleCompareSelected = () => {
    ...
    setComparingCandidates(selectedCandidatesList);
  };
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      {comparingCandidates.length > 0 && (
        <div className="mb-8">
          <CandidateComparison
            candidates={comparingCandidates.map((candidate) => ({
              resumeId: candidate.resumes[0]?.id || "",
              candidateId: candidate.id,
              name:
                candidate.resumes[0]?.structuredData?.contact_info?.name ||
                "Unknown Candidate",
              scores: {
                keywordScore:
                  candidate.resumes[0]?.evaluation?.keywordScore || 0,
                totalScore: candidate.resumes[0]?.evaluation?.totalScore || 0,
              },
              structuredData: candidate.resumes[0]?.structuredData || {
                contact_info: {},
                experience: [],
                education: [],
                skills: {},
                  ...
                  <div className="flex items-start">
                    ...
                    <div className="ml-3">
                      ...
                      <div className="mt-2 text-sm text-green-700">
                        <p>
                          All resume processing jobs have completed. You can now
                          organize candidates into buckets.
                        </p>
                      </div>
                    </div>
                  </div>
        ...
        {isAddingBucket && (
          <div className="fixed inset-0 bg-gray-500 bg-opacity-75 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg p-6 max-w-md w-full">
              <h3 className="text-lg font-medium mb-4">Create New Bucket</h3>
